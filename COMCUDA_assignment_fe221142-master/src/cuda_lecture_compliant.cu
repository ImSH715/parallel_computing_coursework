...
__device__ __forceinline__ unsigned short window_mask_from_shared(const unsigned char tile[GLIDER_BLOCK_Y + 2][GLIDER_BLOCK_X + 2], int tx, int ty) {
    unsigned short mask = 0;
...
/* Lecture 07a: Warp-level reduction using __shfl_down_sync.
   This avoids shared memory bank conflicts during the reduction phase
   by performing the reduction within registers instead of shared memory. */
...
__global__ void count_gliders_kernel(const unsigned char *__restrict__ cells,
                                     int width,
                                     int height,
                                     unsigned long long *__restrict__ count) {
    __shared__ unsigned char tile[GLIDER_BLOCK_Y + 2][GLIDER_BLOCK_X + 2];
    /* Lecture 07a: Replaced 256-entry shared reduction array with 8-entry
       warp sum buffer. 16x16 block = 256 threads = 8 warps.
       This eliminates bank conflicts in the reduction phase. */
    ...
    // Cooperative loading of tile + halo into shared memory
    for (int load_y = ty; load_y < blockDim.y + 2; load_y += blockDim.y) {
        ...
        for (int load_x = tx; load_x < blockDim.x + 2; load_x += blockDim.x) {
            ...
            tile[load_y][load_x] = (gx < width && gy < height) ? cells[(size_t)gy * (size_t)width + (size_t)gx] : 0u;
        }
    }
...
#pragma unroll
    ...
    /* Lecture 07a: Perform reduction within each warp using registers.
       Each warp reduces its 32 'found' values to a single sum. */
    const int lane = linear_tid % 32;
    const int warp_id = linear_tid / 32;
    const unsigned int warp_sum = warp_reduce_sum(found);

    // Write warp sum to shared memory (one entry per warp)
    if (lane == 0) {
        warp_sums[warp_id] = warp_sum;
    }
    __syncthreads();

    /* Final reduction: warp 0 reads the 8 warp sums and reduces them
       to a single block sum, then atomically adds to global counter. */
    if (warp_id == 0) {
        unsigned int val = (lane < (GLIDER_BLOCK_X * GLIDER_BLOCK_Y / 32)) ? warp_sums[lane] : 0u;
...
uint64_t cuda_countGliders(const unsigned char *cells, const size_t width, const size_t height) {
    ...
    cuda_check(cudaMalloc((void **)&d_cells, cells_bytes), "cudaMalloc d_cells");
    cuda_check(cudaMalloc((void **)&d_count, sizeof(unsigned long long)), "cudaMalloc d_count");
    cuda_check(cudaMemcpy(d_cells, cells, cells_bytes, cudaMemcpyHostToDevice), "cudaMemcpy cells");
    ...
    const dim3 grid((unsigned int)(((width - 2) + block.x - 1u) / block.x),
                    (unsigned int)(((height - 2) + block.y - 1u) / block.y));

    count_gliders_kernel<<<grid, block>>>(d_cells, (int)width, (int)height, d_count);
    cuda_check(cudaGetLastError(), "count_gliders_kernel launch");
    cuda_check(cudaDeviceSynchronize(), "count_gliders_kernel sync");

    cuda_check(cudaMemcpy(&h_count, d_count, sizeof(unsigned long long), cudaMemcpyDeviceToHost), "cudaMemcpy count");
...
/* Lecture 05: __ldg() hints the compiler to load through the read-only cache,
   which can improve performance when all threads read the same read-only data. */
__global__ void histogram_kernel(const int *__restrict__ numbers,
                                 int length,
                                 int bin_width,
                                 int histogram_len,
    ...
    for (int i = tid; i < length; i += stride) {
        /* Lecture 05: Use __ldg() for read-only loads to exploit the
           read-only/texture cache and reduce L2 cache pressure. */
...
size_t cuda_histogram(const int *numbers, const size_t length, const int bin_width, int *output) {
    ...
    cuda_check(cudaMemcpy(d_numbers, numbers, numbers_bytes, cudaMemcpyHostToDevice), "cudaMemcpy numbers");
    cuda_check(cudaMemset(d_histogram, 0, histogram_bytes), "cudaMemset histogram");

    int blocks = (int)((length + HISTOGRAM_THREADS - 1u) / HISTOGRAM_THREADS);
    if (blocks < 1) blocks = 1;
    if (blocks > 1024) blocks = 1024;

    histogram_kernel<<<blocks, HISTOGRAM_THREADS>>>(d_numbers,
                                                    (int)length,
                                                    bin_width,
                                                    (int)histogram_len,
                                                    d_histogram);
    ...
    cuda_check(cudaMemcpy(output, d_histogram, histogram_bytes, cudaMemcpyDeviceToHost), "cudaMemcpy histogram");
...
__global__ void emboss_kernel(const unsigned char *__restrict__ pixels,
                              int width,
                              int height,
...
void cuda_emboss(const unsigned char *pixels, const size_t width, const size_t height, unsigned char *output) {
    ...
    cuda_check(cudaMalloc((void **)&d_pixels, pixels_bytes), "cudaMalloc d_pixels");
    cuda_check(cudaMalloc((void **)&d_output, output_bytes), "cudaMalloc d_output");
    cuda_check(cudaMemcpy(d_pixels, pixels, pixels_bytes, cudaMemcpyHostToDevice), "cudaMemcpy pixels");
    ...
    const dim3 grid((unsigned int)((out_w + block.x - 1) / block.x),
    ...
    cuda_check(cudaMemcpy(output, d_output, output_bytes, cudaMemcpyDeviceToHost), "cudaMemcpy output");
