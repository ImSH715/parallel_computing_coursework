...
static inline unsigned short load_window_mask(const unsigned char* row0, const unsigned char* row1, const unsigned char* row2, int x) {
    unsigned short mask = 0;

    if (row0[x + 0]) mask |= (unsigned short)(1u << 0);
    if (row0[x + 1]) mask |= (unsigned short)(1u << 1);
    if (row0[x + 2]) mask |= (unsigned short)(1u << 2);
    if (row1[x + 0]) mask |= (unsigned short)(1u << 3);
    if (row1[x + 1]) mask |= (unsigned short)(1u << 4);
    if (row1[x + 2]) mask |= (unsigned short)(1u << 5);
    if (row2[x + 0]) mask |= (unsigned short)(1u << 6);
    if (row2[x + 1]) mask |= (unsigned short)(1u << 7);
    if (row2[x + 2]) mask |= (unsigned short)(1u << 8);
...
#pragma omp parallel for reduction(+:count) schedule(static)
    for (y = 0; y < height2; ++y) {
        ...
        int x;
        for (x = 0; x < width2; ++x) {
            const unsigned short window_mask = load_window_mask(row0, row1, row2, x);
            int i;
            for (i = 0; i < 16; ++i) {
...
#pragma omp parallel for schedule(static)
    for (y = 0; y < OUT_HEIGHT; ++y) {
        ...
        for (x = 0; x < OUT_WIDTH; ++x) {
            ...
            float pixel_sum = 0.0f;
            /* kx outer, ky inner — matches CPU accumulation order */
            pixel_sum += grey[row0 + sx + 0u] * EMBOSS_KERNEL[0][0];
            pixel_sum += grey[row1 + sx + 0u] * EMBOSS_KERNEL[1][0];
            pixel_sum += grey[row2 + sx + 0u] * EMBOSS_KERNEL[2][0];
            pixel_sum += grey[row0 + sx + 1u] * EMBOSS_KERNEL[0][1];
            pixel_sum += grey[row1 + sx + 1u] * EMBOSS_KERNEL[1][1];
            pixel_sum += grey[row2 + sx + 1u] * EMBOSS_KERNEL[2][1];
            pixel_sum += grey[row0 + sx + 2u] * EMBOSS_KERNEL[0][2];
            pixel_sum += grey[row1 + sx + 2u] * EMBOSS_KERNEL[1][2];
            pixel_sum += grey[row2 + sx + 2u] * EMBOSS_KERNEL[2][2];
